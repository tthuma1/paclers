...
def create_team(first_index, second_index, is_red, first='ReflexCaptureAgent', second='CustomUniversalAgent', num_training=0):
    """
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  """
    ...
    def get_features(self, game_state, action):
        ...
        if len(
            food_list) > 0:  # This should always be True,  but better safe than sorry
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def get_features(self, game_state, action):
        ...
        enemies = [successor.get_agent_state(i) for i in
                   self.get_opponents(successor)]
        invaders = [a for a in enemies if
                    a.is_pacman and a.get_position() is not None]
        features['num_invaders'] = len(invaders)
        if len(invaders) > 0:
            dists = [self.get_maze_distance(my_pos, a.get_position()) for a in
                     invaders]
